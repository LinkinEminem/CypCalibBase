...
namespace apipro
{
    ...
    int TAPIPro::Connect(const char* ipAddress, const int devNum, CallbackType callback, void* taskID)
    {
    ...
    int TAPIPro::Disconnect()
    {
        return FApiDev.DisConnect();
    }

    int TAPIPro::Home(const int smrSize)
    {
        return FApiDev.Home(smrSize);
    }

    bool TAPIPro::IsConnected()
    {
        return FApiDev.IsConnected();
    }

    int TAPIPro::GetSingleMeasure(const int interval, double* rstValue)
    {
    ...
    int TAPIPro::IsBeamLocked()
    {
        REALTIME_INFO info;
        const int ret = FApiDev.getRTData(info);
        if (ret == API_ERROR_SUCCESS)
        {
            return !info.Laser_Path_Error && !info.Laser_Dist_Error && (info.Operation_Mode == OP_Track);
        }
        return ret; 
    }
    
    int TAPIPro::JogTo(float azimuth, float elevation)
    {
        TARGET_EXTENDED jog;
        jog.JogMode = false;
        jog.InPosition = true;
    ...
    int TAPIPro::TargetSearch(const float distance, const float radium, const float waitTime)
    {
        return FApiDev.TargetSearch(distance, radium, waitTime);
    }
    
    bool TAPIPro::GetAPIErrorMessage(const int errID, char* errMsg)
    {
        return FApiDev.getAPIErrorMessage(errMsg, errID);
    }
    
    int TAPIPro::SetTrackMode()
    {
